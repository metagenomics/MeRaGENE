...
while read line; do 
	...
	if [ $LEN != "0" ] 
		...
		#search for keywords
		pubtxt=$(grep -e "metallo" -e "carbapenemase" -e "zeffalospurin" -e "antibiotic" -e "resistance" -e "lactamase" -e "plasmid" -e "esbl" $OUTPUT/$accession.pubmed)
		...
		if [ $LENGTH != "0" ] 
			...
			while read pub; do
				...
				then
					...
					pubtxt=$(grep -e "metallo" -e "carbapenemase" -e "zeffalospurin" -e "antibiotic" -e "resistance" -e "lactamase" -e "plasmid" -e "esbl" -e "betalaktamase" $OUTPUT/$accession.pubmed)
