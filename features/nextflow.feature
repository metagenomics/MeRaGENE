Feature: Verification steps for bioPipeline
  ...
  Scenario: Run the pipeline with valid parameters
    ...
    When I run the command:
      """
      nextflow run metagenomics/bioPipeline   -profile 'local'  --GENOME="$PWD/tmp/db.faa"   --NCBI="$PWD/tmp/blast.db"   --BLASTP="blastp" --BLAST_CPU=2 --HMM_SEARCH="hmmsearch"  --HMM_SCAN="hmmscan" --HMM_PRESS="hmmpress"   --INPUT="$PWD/tmp/hmm"   --OUTPUT="$PWD/tmp/output"   --cov="$PWD/tmp/test.bam.coverage.txt"
      """
