Feature: Verification steps for MeRaGENE
  ...
  Scenario Outline: Run the pipeline without coverage files
    Given I copy the example data files:
      | source           | dest        |
      | db.faa           | db.faa      |
      | blast.db         | blast.db    |
    ...
    And the file "output/overview_new.txt" should contain 6 lines
    Examples:
      | faa                              | blast                        | input                     | output                       | search                             | keywords                             |
      | --genome="${PWD}/tmp/db.faa"     | --ncbi="${PWD}/tmp/blast.db" |  --input="${PWD}/tmp/hmm" | --output="${PWD}/tmp/output" | --search="${PWD}/tmp/search.yaml"  | --keywords="${PWD}/tmp/keywords.txt" |
      | --genome="db.faa"                | --ncbi="blast.db"            |  --input="hmm"            | --output="output"            | --search="search.yaml"             | --keywords="keywords.txt"  |
